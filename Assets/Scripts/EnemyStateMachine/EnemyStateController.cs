...
namespace EnemyStateMachine
{
    public class EnemyStateController
    {
        ...
        private readonly float _triggeredDistance;
        private readonly float _fov;
        private readonly float _attackDistance;

        public EnemyStateController(EnemyStateMachine context)
        {
            _target = context.Target;
            _enemy = context.EnemyEntity.Rigidbody.transform;
        ...
        public bool CheckIfFindTarget(Vector2 enemyDirection)
        {
            ...
            if (Vector2.Angle(enemyDirection, directionToTarget) > _fov / 2f &&
                distanceToTarget > _triggeredDistance)
                return false;
